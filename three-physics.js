...
export class Spring {
  ...
  updateLine() {
    if (this.line && this.particle_1 && this.particle_2) {
      ...
      positions.setXYZ(
        0,
        this.particle_1.pos.x,
        this.particle_1.pos.y,
        this.particle_1.pos.z
      );
      positions.setXYZ(
        1,
        this.particle_2.pos.x,
        this.particle_2.pos.y,
        this.particle_2.pos.z
      );
  ...
  computeForces() {
    ...
    for (const p of this.particles) {
      ...
      if (p.pos.y < this.ground_y) {
        ...
        const spring_force = new THREE.Vector3(0, 1, 0).multiplyScalar(
          depth * this.ground_ks
        );
        p.force.add(spring_force);

        // Damping force
        if (p.vel.y < 0) {
          const damping_force = new THREE.Vector3(0, 1, 0).multiplyScalar(
            -p.vel.y * this.ground_kd
          );
...
export class BallPhysics {
        ...
        if (this.sphereIntersectsBox(ball.position, radius, box)) {
          ...
          if (collider.type === "glass") {
            ...
            closestPoint.x = Math.max(
              box.min.x,
              Math.min(ball.position.x, box.max.x)
            );
            closestPoint.y = Math.max(
              box.min.y,
              Math.min(ball.position.y, box.max.y)
            );
          ...
          const penetrationDir = new THREE.Vector3()
            .subVectors(ball.position, closestPoint)
            .normalize();
          const penetrationDepth =
            radius - ball.position.distanceTo(closestPoint);
          ...
          if (penetrationDepth > 0) {
            ...
            const bounceCoef =
              this.calculateAdaptiveRestitution(speedAlongNormal);
  ...
  handleGlassHoleCollision(ball, glassBox) {
    ...
    if (this.sphereIntersectsBox(ball.position, radius, glassBox)) {
      ...
      closestPoint.x = Math.max(
        glassBox.min.x,
        Math.min(ball.position.x, glassBox.max.x)
      );
      closestPoint.y = Math.max(
        glassBox.min.y,
        Math.min(ball.position.y, glassBox.max.y)
      );
      closestPoint.z = Math.max(
        glassBox.min.z,
        Math.min(ball.position.z, glassBox.max.z)
      );
      ...
      if (penetrationDepth > 0) {
        ...
        const dampingForce =
          Math.max(0, velocityAlongNormal) * this.wallDampingConstant;
        ...
        velocity.addScaledVector(
          penetrationDir,
          -acceleration * this.dt * restitution
        );
  ...
  handleBallEffectorCollision(ball1, eff, effPos, effVel) {
    ...
    if (distance < combinedRadius) {
      ...
      const normal = new THREE.Vector3()
        .subVectors(ball1.position, effPos)
        .normalize();
  ...
  handleBallCollision(ball1, ball2) {
    if (!ball1.userData.boundingSphere || !ball2.userData.boundingSphere)
      return;
    ...
    if (distance < combinedRadius) {
      ...
      if (isImmobile1 && isImmobile2) {
        ...
        const restitution = this.calculateAdaptiveRestitution(
          Math.abs(dotProduct)
        );
        ...
        const restitution =
          this.calculateAdaptiveRestitution(relVelocityMagnitude);
  ...
  checkBallBounds(ball, referencePos, maxDistance, resetFunction) {
    ...
    if (distanceFromCenter > maxDistance) {
      //   console.log(
      //     `Ball ${ball.name} has fallen out of bounds, resetting position`
      //   );
  ...
  calculateAdaptiveRestitution(relativeVelocityMagnitude) {
    ...
    // For slow movements, linearly reduce restitution
      ...
      return (
        0.1 +
        ((baseRestitution - 0.1) *
          (relativeVelocityMagnitude - this.restVelocityThreshold)) /
  ...
  applyVelocityThreshold(velocity) {
    ...
    if (
      speedSquared <
      this.restVelocityThreshold * this.restVelocityThreshold
    ) {
      velocity.set(0, 0, 0);
...
export class BoundingBoxCollider {
  constructor(min, max, color = 0xffff00) {
    ...
    // console.log(
    //   `Created BoundingBoxCollider at (${min.x}, ${min.y}, ${min.z}) to (${max.x}, ${max.y}, ${max.z})`
    // );
  }
  ...
  setFromMinMax(min, max) {
    ...
    this.updateHelper();
    // console.log(
    //   `Updated BoundingBoxCollider to (${min.x}, ${min.y}, ${min.z}) to (${max.x}, ${max.y}, ${max.z})`
    // );
    return this;
  }
  ...
  setFromCenterAndSize(center, size) {
    ...
    this.box.set(center.clone().sub(halfSize), center.clone().add(halfSize));
    this.updateHelper();
    return this;
  }
