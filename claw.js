...
export class ClawScene {
	constructor() {
		...
		this.initCustomClaw(1, 0.15, Math.PI / 3);
		this.initToys();
	...
	initScene() {
		...
		this.camera.position.set(-14, 20, 30);
	...
	initCameraControls() {
		try {
			...
			this.controls.target.set(-14.4, 3, 21.5);
	...
	updateToyPhysics(dt, time) {
			...
			for (let j = i + 1; j < this.toys.length; j++) {
				if (this.toys[i] && this.toys[j]) {
					...
					if (
						...
						!this.toys[j].userData ||
						!this.toys[i].userData.boundingSphere ||
						!this.toys[j].userData.boundingSphere ||
						!this.toys[i].userData.velocity ||
						!this.toys[j].userData.velocity
					) {
						continue;
					}
		...
		for (const toy of this.toys) {
			...
			if (this.machineBounds) {
				hasMainCollision = this.ballPhysics.handleBoxCollision(
					toy,
					this.machineBounds
				);
			}
			...
			if (toy.position.y < 2.5) {
				// Floor is around y=2.85
				this.resetBallPosition(toy);
			}
			...
			if (glassBox && this.ballsInGlassHole.includes(toy)) {
				...
				const distanceFromBottom =
					toy.position.y - (glassBox.min.y + sphere.radius);
  ...
  toggleClaw() {
	...
	setupIkJointsFromModel() {
		...
		const cylinder001 = this.findObjectByName(
			this.clawMechanism,
			"cylinder.001"
		);
	...
	handleBarrierCollision(ball) {
		...
		if (sphere.intersectsBox(barrierBox)) {
			...
			if (velocityAlongNormal < 0) {
				...
				ball.userData.velocity.addScaledVector(
					normal,
					-1.8 * velocityAlongNormal
				);
